...
function Base.iterate(iter::FrozenSortedArrayIndex{K}, state::Int=1)::Union{Nothing, Tuple{Pair{K, Int}, Int}} where {K}
    state > length(iter.keys) && return nothing
    return ((iter.keys[state] => state), state+1)
end

Base.eltype(iter::Type{FrozenSortedArrayIndex{K}}) where K = Pair{K, Int}

Base.length(iter::FrozenSortedArrayIndex{K}) where K = length(iter.keys)

Base.keys(iter::FrozenSortedArrayIndex{K}) where K = iter.keys
