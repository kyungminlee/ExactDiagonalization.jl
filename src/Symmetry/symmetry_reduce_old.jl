...
function symmetry_reduce_serial_old(
  ...
  return ReducedHilbertSpaceRepresentation{HSR, BR, ComplexType}(hsr, trans_group, reduced_basis_list, [x.index for x in basis_mapping], [x.amplitude for x in basis_mapping])
end



function symmetry_reduce_parallel_old(
    hsr ::HilbertSpaceRepresentation{QN, BR, DT},
    trans_group ::TranslationGroup,
    fractional_momentum ::AbstractVector{<:Rational},
    complex_type::Type{ComplexType}=ComplexF64;
